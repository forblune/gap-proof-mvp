...
// 부정 판정은 두 갈래로 본다. 어느 쪽도 "절 어디에나 있으면 부정" 으로 취급하지 않는다.
//
// 3라운드에 걸쳐 배운 것:
//  - 창을 넓히면 실제로 한 일이 지워진다("장애가 한 번도 없었습니다" 는 키워드에서 17자).
//  - 창을 없애면 다른 명사에 붙은 부정을 가져온다("…전무한테 보고했습니다" 는 37자).
//  - 그래서 재현율은 **창을 늘려서** 가 아니라 **어휘와 어미를 정확히 해서** 올린다.
//
// 1단 — 근접 판정. 흔한 부정 어휘라 거리로 소속을 가린다(키워드에서 24자 안).
const NEGATION_NEAR_PATTERNS: RegExp[] = [
  ...
  // "경험/경력/지식/능력이 없다" — "누락 없음" 같은 다른 명사의 부재와 섞이지 않도록 명사를 못박는다.
  /(경험|경력|지식|능력)[이은도]?\s*[:：]?\s*(전혀|거의|아직)?\s*없/,
  // "해보지 않았다 / 해 본 적 없다 / 안 해 봤다" 계열.
...
// 2단 — 절의 서술어 자리에 온 부정만 본다. 한국어는 서술어가 절 끝에 오고 절 전체를 지배하므로,
// **절 끝에 붙어 있을 때만** 키워드까지 지배한다고 본다($ 앵커가 그 조건이다).
//
// 앵커가 없으면 무엇이 깨지는지(심사에서 실제로 나온 반례):
//   "경험 제로에서 시작해 3년 만에 팀을 맡았습니다"  → 제로 뒤에 말이 이어지면 부정이 아니다
//   "남들이 못 한다고 하던 걸 제가 했습니다"          → 못 한다 뒤에 말이 이어지면 부정이 아니다
//   "전공이 아니어서 더 열심히 했습니다"              → 아니 뒤에 말이 이어지면 부정이 아니다
const NEGATION_FINAL_PATTERNS: RegExp[] = [
  // "미경험" — "미경험자를 교육했습니다" 의 미경험자는 남을 가리키므로 제외한다.
  /미경험(?!자)/,
  /(경험|경력)\s*[:：]?\s*제로\s*$/,
  // "경험은 전무합니다" — 직함 "전무"(전무님·전무한테·전무가…)와 갈라낸다.
  // "전무한 수준입니다" 처럼 서술어가 두 어절에 걸치므로 절 끝까지 한글 어절만 이어지는 것을 허용한다.
  // \b 는 ASCII 기준이라 한글 뒤에서는 동작하지 않는다 — 조사는 그냥 열거해서 막는다.
  // 계사 '입'은 뺐다 — "결재한 분은 물류본부 전무입니다" 가 지워졌기 때문이다.
  /전무(?!님|한테|에게|가|는|를|와|랑|께)(하|한|합|했|해)(?:[가-힣]|\s+[가-힣])*\s*$/,
  // "못합니다/못해요/못한다" — 절 끝에 온 종결형만. "못 하던 일을 제가 만들었습니다" 는 아니다.
  /(^|[^잘])못\s?(합니다|해요|한다|했습니다)\s*$/,
  // "모릅니다/모르겠습니다/아무것도 모릅니다" — "모르는·모르던 동료" 같은 관형형은 제외한다.
  /(모릅니다|몰라요|몰라\s*$|모르겠[습어다][가-힣]*|(전혀|하나도|아무것도)\s*모르(?!는|던|시)[가-힣]*)\s*$/,
  /손도\s*못\s*(대|댑|댔)[가-힣]*\s*$/,
  /젬병[가-힣]*\s*$/,
  /(맡은|담당한)\s*(적|일)[이은]?\s*없[가-힣]*\s*$/,
  /문외한[가-힣]*\s*$/,
  // "제 분야가/담당이 아닙니다" — "쪽" 은 "디자인 쪽이 아니라 개발을 했습니다" 때문에 뺐다.
  /(영역|분야|전공|담당)[이가은]?\s*아(니|닙)[가-힣]*\s*$/,
];
...
function clausesOf(text: string): string[] {
  return text
    ...
    // 실제로 한 일까지 지워진다.
    // 한국어에서 가장 흔한 연결어미 -고/-며/-면서 도 자른다. 이걸 빼먹으면
    // "재고 데이터를 매일 정리하고 개발은 못합니다" 가 한 절이 되어, 절 끝의 "못합니다" 가
    // 앞의 '데이터' 까지 지배한다 — 실제로 한 일이 지워진다(심사에서 나온 재현).
    //
    // 과하게 자르는 쪽은 대체로 안전하다. 절이 잘게 나뉘면 키워드가 든 절에 부정이 없어져
    // "증거로 센다" 쪽으로 기운다. 놓치는 것보다 지우는 쪽이 나쁘다는 이 파일의 원칙과 맞다.
    //
    // 다만 -어서/-아서 는 자르지 않는다. 앞뒤 주어가 이어지면서 뜻이 뒤집히는 자리라
    // "개발이 전공이 아니어서 더 열심히 파고들었습니다" 를 자르면 앞 조각만 남아 부정이 된다.
    .split(/[.!?\n·]|,\s*|지만|(?<=[가-힣])으나|(?<=[가-힣])나\s|는데|(?:그리고|하지만|그러나|다만)|(?<=[가-힣])고\s|(?<=[가-힣])며\s|(?<=[가-힣])면서/)
    .map((part) => part.trim())
    .filter(Boolean);
}
...
export function keywordIsNegated(text: string, keyword: string): boolean {
  ...
  return clauses.every((clause) => {
    ...
    return isNegatedPredicate(after);
  });
}
...
function clauseIsNegation(clause: string): boolean {
  return isNegatedPredicate(clause);
}

// 주어진 서술부가 부정인가. 근접 패턴은 앞 24자에서만, 서술어 패턴은 절 끝에서만 본다.
function isNegatedPredicate(after: string): boolean {
  if (NEGATION_FINAL_PATTERNS.some((pattern) => pattern.test(after))) return true;
  const near = after.slice(0, NEGATION_WINDOW);
  return NEGATION_NEAR_PATTERNS.some((pattern) => pattern.test(near));
}
