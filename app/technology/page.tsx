...
const SECURITY = [
  { name: "접근 게이트", detail: "접근 코드를 서버가 검증하고 서명된 HttpOnly 쿠키 세션을 발급해요. 환경변수가 없으면 데모가 열리지 않는 fail-closed 방식이에요." },
  ...
  { name: "무저장 원칙", detail: "경험 원문을 서버에 저장하지 않아요. 기록 삭제는 화면의 모든 상태를 지워요." },
];
  ...
  return (
    ...
    >
      ...
      <section className="info-section">
        <h2>검증 방법</h2>
        <ul>
          <li>자동 테스트 14종: 입력 경계값(19·20·3,000·3,001자), 게이트 인증·위조 쿠키·fail-closed, 요청 한도, 개인정보 가리기, 모델 허용 목록, 서버 렌더링.</li>
          <li>실제 Worker 런타임(workerd)에서 게이트 세션·한도·보안 헤더를 실측했어요.</li>
          <li>360·390·768·1024·1440px 다섯 뷰포트에서 가로 넘침 0, 콘솔 오류 0을 자동으로 확인해요.</li>
